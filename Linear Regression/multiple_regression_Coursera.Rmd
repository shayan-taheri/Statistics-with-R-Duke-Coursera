...
summarise(evals, mean_score=mean(score),med_score=median(score),min_score=min(score),max_score=max(score))
...
```{r jitter}
# type your code for the Exercise here, and Knit, points only ploted integer.
ggplot(data = evals, aes(x = bty_avg, y = score)) +
  geom_jitter()
```
...
```
```{r eval=FALSE}
m4 = lm(score ~ ethnicity + rank + gender + language + age + cls_perc_eval + cls_level + cls_profs + cls_credits+ bty_avg, data = evals)
summary(m4)$adj.r.squared
```
```{r eval=FALSE}
m5 = lm(score ~ ethnicity + rank + gender + language + age + cls_perc_eval + cls_students + cls_level + cls_credits+ bty_avg, data = evals)
