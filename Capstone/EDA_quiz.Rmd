...
ames_train %>%
  count(Fireplace.Qu== "NA")

ames_train %>%
  count(Lot.Frontage== "NA")
```
...
str(ames_train)
# based on the codebook, MS.SubClass, Overall.Qual and Overall.Cond are all factors, however, the answer to the question is 2 and the quiz answer said Overall.Qual and Overall.Cond are ratings, they are not categorical.
```
...
summarise(count_neib= n(),mean_neib = mean(price), sd_neib= sd(price) )
...
ames_train_w2 <- ames_train %>%
mutate(area_log=log(area), price_log=log(price))

ggplot(data = ames_train_w2, aes(x = area_log, y = price_log)) +
  geom_jitter()
...
ames_train %>%
  summarise(year_ratio = sum(Year.Built >1999) / n(), mean_price = mean(price), median_price=median(price), no_basement= sum(Bsmt.Cond =="NA"), gravel_street=sum(Street=="Grvl"))

ames_train %>%
group_by(Bsmt.Qual) %>%
summarise(count_bsmt_qual= n())
...
ames_train_w2 <- ames_train_w2 %>%
mutate(garage= ifelse(Garage.Cars==0, "no", "yes"))
inference(y = Lot.Area, x = garage, data = ames_train_w2, statistic = "mean", type = "ht", null = 0, 
...
ames_train_w2_q9 <- ames_train_w2 %>%
filter(area > 2000)
table(ames_train_w2_q9$Bedroom.AbvGr)
```

```{r}
alpha=9+ 1*2+2*6+3*53+4*61+5*12+6*4
beta= 3+2+6+53+61+12+4
...
plot_ss(x = area_log, y = price_log, data = ames_train_w2, showSquares = TRUE)
```
...
library(plyr)
count(ames_train_w2, c("Neighborhood", "Bldg.Type"))
```
...
ggplot(data = ames_train_w2, aes(x = area_log, y = Bedroom.AbvGr)) +
  geom_jitter()
...
ames_train_w2_q14 <- ames_train_w2 %>%
filter(Bsmt.Unf.SF > 0)

ames_train_w2_q14 %>%
summarise(mean_Bsmt.Unf.SF= mean(Bsmt.Unf.SF))
