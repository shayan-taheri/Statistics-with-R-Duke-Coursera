...
ames_train_w5 <- ames_train %>%
  mutate(price_log=log(price), area_log=log(area), lot.area_log= log(Lot.Area))

```

```{r}
full_model = lm(price_log ~ area_log + lot.area_log + Bedroom.AbvGr + Overall.Qual + Land.Slope, data = ames_train_w5)
ames_train_w5_aic = stepAIC(full_model,data = ames_train_w5, direction="backward")
summary(ames_train_w5_aic)
```

```{r}
ames_train_w5_bic = stepAIC(full_model,data = ames_train_w5, direction="backward", k=log(nrow(ames_train_w5)))
summary(ames_train_w5_bic)
```
...
model_q3 = lm(price_log ~ area_log, data = ames_train_w5)
summary(model_q3)
model_q3_resid= resid(model_q3)

cbind(ames_train_w5,model_q3_resid) %>%
  group_by(Neighborhood) %>%
  summarise(mean_resid=mean(model_q3_resid)) %>%
...
model_q5 = lm(price_log ~ Overall.Qual+ Bsmt.Qual + Garage.Qual, data = ames_train_w5)
summary(model_q5)
```
...
ames_train_w5$MS.SubClass <- as.factor(ames_train_w5$MS.SubClass)
str(ames_train_w5$MS.SubClass)

model_q7 = lm(price_log ~  Overall.Cond + Overall.Qual, data = ames_train_w5)
model_q7_predict = predict(model_q7,ames_train_w5)
  cbind(ames_train_w5, model_q7_predict) %>%
  group_by(MS.SubClass) %>%
  summarise(median_predict=median(model_q7_predict)) %>%
  arrange(desc(median_predict))

```

```{r Q7 alt}
model_q7_fitted = model_q7$fitted.values
  cbind(ames_train_w5, model_q7_fitted) %>%
  group_by(MS.SubClass) %>%
  summarise(median_fitted=median(model_q7_fitted)) %>%
  arrange(desc(median_fitted))
...
which.max(abs(hatvalues(model_q7)))
...
model_q12 = lm(price_log ~ area_log, data = ames_train_w5)
summary(model_q12)
```
