...
price_predict_AIC = exp(predict(model.AIC,ames_test))
mean_train= mean(price_predict_AIC)
mean_train
```
...
# full test rmse= 24197.34


#aic test rmse =24188.71
resid.aic.test <- ames_test$price - price_predict_AIC
rmse.aic.test <- sqrt(mean(resid.aic.test ^2))
rmse.aic.test

#full train rmse = 22706.55
predict.full_train <- exp(predict(model.full, ames_train))
resid.full.train <- ames_train$price - predict.full_train
rmse.full.train <- sqrt(mean(resid.full.train^2))
rmse.full.train

#aic train rmse= 22823.41
predict.aic_train <- exp(predict(model.AIC, ames_train))
resid.aic.train <- ames_train$price - predict.aic_train
...
#RMSE of HPM = 24188.71
#RMSE of BPM = 24189.98
